...
The general asynchronous counter connects T Flip-Flops (`TFF`) in a daisy-chain configuration with `T = 1`. Since each stage toggles only when triggered by the preceding stage's output, propagation delay accumulates proportionally with every additional bit ($N \times t_{pd}$).
...
The 3-bit up counter cascades three flip-flops (`XLXI_1`, `XLXI_2`, `XLXI_3`) to count sequentially upwards in binary from `000` ($0_{10}$) to `111` ($7_{10}$), resetting to `000` on the 8th clock pulse.
...
The 3-bit down counter configures inverted output clocking ($\bar{Q}$) using internal net drivers (`XLXN_3`, `XLXN_4`) to count sequentially downwards in binary from `111` ($7_{10}$) to `000` ($0_{10}$), wrapping around back to `111`.
...
* **Signal Pinout**:
  ...
  * `CLR` / `Clr`: Asynchronous Clear / Reset signal (forces outputs $Q_2 Q_1 Q_0 = 000$).
  * `T`: Toggle control line (tied HIGH to enable toggling on every active clock edge).
  * `Q0`, `Q1`, `Q2`: 3-bit binary counter outputs ($Q_0$ = LSB, $Q_2$ = MSB).
  * `Q0_DUMMY`, `Q1_DUMMY`: Intermediate stage output buffers driving subsequent flip-flop clock inputs in schematic capture.
  * `XLXN_3`, `XLXN_4`: Internal complementary output nets ($\bar{Q}$) used for down-counter clock triggering.
